...
async def get_category(
    ...
    await callback.message.edit_reply_markup(
        reply_markup=get_paginator_keyboard(data=categories)
        )
...
async def search_category(callback: CallbackQuery, state: FSMContext):
    page = int(callback.data.split('_')[1])
    data = await state.get_data()
    categories = data.get('all_category_goods', [])
    await callback.message.edit_reply_markup(
        reply_markup=get_paginator_keyboard(page=page, data=categories)
        ) 
...
async def get_subcategory(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    await callback.message.edit_reply_markup(
        reply_markup=get_paginator_keyboard(data=subcategories)
        )
...
async def confirm(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        data = await state.get_data()
        product_name = await orm_get_product_name_for_id(
            session=session,
            id=data['select_product']
            )
...
async def confirm(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    data = await state.get_data()
    cart = await orm_create_cart(
        session=session,
        user_id=callback.from_user.id,
        product_id=data['select_product'],
        count=data['count']
        )
    print(cart.id)
    await callback.message.answer(
        f"Товар: {data['confirm']} добавлен в корзину в количество {data['count']}",
